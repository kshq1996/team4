...
<%-- <jsp:forward page="ProductServlet?method=listProductByPage&page=1&count=12"></jsp:forward> --%>
<%-- </c:if> --%>
<head>
<base href="<%=basePath %>" />
...
<script type="text/javascript" scr="js/json.js"></script>
</head>
		...
		<div class="search-list">
			...
			<div class="am-g am-g-fixed">
				<div class="am-u-sm-12 am-u-md-12">
					<div class="theme-popover">
						<div class="searchAbout">
							<span class="font-pale">相关搜索：</span> <a title="坚果" href="#">坚果</a> <a title="瓜子" href="#">瓜子</a> <a title="鸡腿" href="#">豆干</a>

						</div>
						<ul class="select">
							<p class="title font-normal">
								<span class="fl">松子</span> <span class="total fl">搜索到<strong class="num">997</strong>件相关商品
								</span>
							</p>
							<div class="clear"></div>
					...
					<div class="search-content">
						...
						<div class="clear"></div>
						<ul class="am-avg-sm-2 am-avg-md-3 am-avg-lg-4 boxes" id="listProduct">
							<%-- 								<% ArrayList<Product> ps=(ArrayList<Product>)request.getAttribute("productByButtonkey");%> --%>
							<%-- 								<%if(ps!=null){ --%>
							<!-- 										for(Product p:ps){ %> -->
							<c:forEach var="ps" items="${requestScope.productByButtonkey}" varStatus="status">
								<li>
									<div class="i-pic limit">
										<a href="ProductServlet?method=getInfoById&id=${ps.productId}"> <img src="${ps.productImg1}" />
											<p class="title fl">${ps.productName}</p>
											<p class="price fl">
												<b>¥</b><strong>${ps.productCost}</strong>
											</p>
										</a>
									</div>
								</li>
							...
							<%-- 								<% }}else{%> --%>
							<!-- 									   <div class="i-pic limit"> -->
							<!-- 											<span color="red">亲，商品跑掉了哦！没找到呢！</span> -->
							<!-- 										</div> -->
							<%-- 								<%} %> --%>
						</ul>
					</div>
					...
					<!-- 						<li><a -->
					<%-- 							href="ProductServlet?method=listByPage&page=${requestScope.page.firstpage}&count=${requestScope.page.count}"">首页</a></li> --%>
					<!-- 						<li class="am-disabled"><a href="#">&laquo;</a></li> -->
					<!-- 						<li class="am-active"><a -->
					<!-- 							href="ProductServlet?method=page&page=1">1</a></li> -->
					<%-- 						<li><a href="ProductServlet?method=page&page=${i}">上一页</a></li> --%>
					<%-- 						<li><a href="ProductServlet?method=page&page=${i}">下一页</a></li> --%>
					<!-- 						<li><a href="#">&raquo;</a></li> -->
					<!-- 					</ul> -->
					<input type="hidden" id="nextpage" value="${requestScope.page.nextPage }"> <input type="hidden" id="count" value="${requestScope.page.count}"> <input type="hidden" id="key" value="${requestScope.key}">
					...
					$(function () {
				      ...
				      $(window).scroll(function(){
				  		...
				  		if(Math.round(scrollTop) + windowHeight >= scrollHeight){ //到达底部100px时,加载新内容  
							        	if($("#info").html()!="没有更多商品了"){
												$("#info").html("<img src='images/systemImages/loading.gif'  width='300' height='150'/>");
												setTimeout(function (){
													var url="ProductServlet?method=listByPage&key="+$("#key").val()+"&page="+$("#nextpage").val()+"&count="+$("#count").val();
													$.get(url,function(data){
													if(data.length>0){
													for(var  x=0;x<data.length;x++){
														var  newProductInfo="<li><div class='i-pic limit' ><a href='ProductServlet?method=getInfoById&id="+data[x].productId+"'><img src='"+data[x].productImg1+"' /><p class='title fl'>"+data[x].productName+"</p><p class='price fl'><b>¥</b><strong>"+data[x].productCost+"</strong></p></a></div></li>";	                                                       						
														$("#listProduct").append(newProductInfo);
													}
				...
				<div style="text-align: center;">
					<span id="info" style="font-size: 12px; color: #36D2B6">滚动加载更多</span>
				</div>
			</div>
