...
<body>
	...
	<div class="nav white">
		...
		<div class="search-bar pr">
			...
			<form>
				<input id="searchInput" name="index_none_header_sysc" type="text" placeholder="搜索" autocomplete="off"> <input id="ai-topsearch" class="submit" value="搜索" index="1" type="submit">
			</form>
		</div>
	</div>
	...
	<div class="take-delivery">
		<div class="status">
			...
			<div class="successInfo">
				<ul>
					<div class="user-info">
						<p>收货人：${requestScope.address.name}</p>
						<p>联系电话：${requestScope.address.phonenum}</p>
						<p>收货地址：${requestScope.address.province} ${requestScope.address.state} ${requestScope.address.city} ${requestScope.address.street} ${requestScope.address.addressdetail}</p>
						<p>订单号：${requestScope.orders.order_id}</p>
						<p>订单时间：${requestScope.orders.date}</p>
					</div>
					请认真核对您的收货信息，如有错误请联系客服
				</ul>
				<div class="option">
					<span class="info">您可以</span> <a href="person/order.html" class="J_MakePoint">查看<span>已买到的宝贝</span></a> <a href="person/orderinfo.html" class="J_MakePoint">查看<span>交易详情</span></a>
				</div>
			</div>
		</div>
	</div>
